...
RTTICompleteObjectLocator* rtti_read_complete_object_locator(const Process* process, uintptr_t addr) {
    uintptr_t colAddress;
    ReadProcessMemory(process->handle, addr, &colAddress, sizeof(uintptr_t), 0);
